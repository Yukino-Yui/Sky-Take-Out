...
public class SetmealServiceImpl implements SetmealService {
    ...
    public PageResult pageQuerySetmeal(SetmealPageQueryDTO setmealPageQueryDTO) {
        // 1. 开启分页（告诉 MyBatis：下一条 select 是分页查询）SQL 里不需要写 limit，PageHelper 会自动加 limit
        PageHelper.startPage(setmealPageQueryDTO.getPage(),setmealPageQueryDTO.getPageSize());

        // 2. 执行 mapper 的 select
    ...
    /**
     *  修改套餐
     *  注意：修改的时候都是执行完update修改，然后delete关联，再insert关联
     * @param setmealDTO
     */
    @Transactional
    public void update(SetmealDTO setmealDTO) {
        ...
        //2、根据套餐id删除套餐和菜品的关联关系，操作setmeal_dish表，执行delete
        setmealDishMapper.deleteBySetmealId(setmealId);
