...
public class RedisConfiguration {
     ...
     * 方法返回一个对象，是bean，会被注入到ioc容器
     * 这里的 redisConnectionFactory 就是 Spring Boot自动生成的
    ...
    public RedisTemplate redisTemplate(RedisConnectionFactory redisConnectionFactory){
        ...
        //设置redis的连接工厂对象,作用：告诉 RedisTemplate 如何连接 Redis。
        //如果不设置：RedisTemplate不知道Redis在哪里，无法建立连接、无法操作Redis
        redisTemplate.setConnectionFactory(redisConnectionFactory);

        /**
         * Redis 默认使用 JDK 序列化（会出现乱码），所以配置了 key 序列化器
